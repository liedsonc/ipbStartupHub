...
export default function InboxPage() {
  const { requests, unreadCount, markAsRead, markAllAsRead, removeRequest } = useInbox();
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Caixa de Entrada de Colaboração</h1>
          <p className="text-gray-600">
            Gerencie solicitações de colaboração para suas ideias.
          </p>
        </div>
        {requests.length > 0 && (
      ...
      {requests.length === 0 ? (
        <EmptyState
          title="Nenhuma solicitação de colaboração"
          description="Quando alguém expressar interesse em colaborar em suas ideias, as solicitações aparecerão aqui."
        />
      ) : (
